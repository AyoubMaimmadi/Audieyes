            ...
            {!bool1 && (
                <div className='result-page'>
                    {localStorage.getItem('token') && (
                        <button onClick={handleLogout} className='result-logout' style={{ position: 'absolute', right: '118px', top: '27px' }}>
                            Logout
                        </button>
                    )}
                    {/* <video id="bg-video" src="../bgvid.mp4" autoplay loop muted></video> */}
                    <div className='result-window' style={{ position: 'reative' }}>
                        <button style={{ color: 'black', marginLeft: '-31rem' }} className='result-logout' onClick={handleClick}>
                            Go back
                        </button>
